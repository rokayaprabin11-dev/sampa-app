...
class HelpContactSupportScreen extends StatefulWidget {
  ...
  @override
  State<HelpContactSupportScreen> createState() =>
      _HelpContactSupportScreenState();
}
...
class _HelpContactSupportScreenState extends State<HelpContactSupportScreen> {
  ...
  Future<void> _submit() async {
    if (_subject.text.trim().isEmpty) {
      showHelpToast(context, 'Add a short subject first',
          icon: Icons.info_outline);
      return;
    }
    if (_description.text.trim().isEmpty) {
      showHelpToast(context, 'Describe the issue first',
          icon: Icons.info_outline);
      return;
    }
    ...
    return HelpScaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const HelpSectionTitle('Category',
          ...
          HelpTextField(
            label: 'Description',
            hint:
                'Tell us what happened, when it started, and what you expected instead',
            maxLines: 5,
          ...
          Center(
              child: Text(
                  'We\'ll reply inside the app under "My Support Requests"',
                  style: TextStyle(
                      fontSize: 11, color: HelpPalette.of(context).faint))),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget btn(IconData icon, String label) => Expanded(
          child: InteractiveSurface(
            onTap: () => showHelpToast(
                context, 'Attach files in your mail app after tapping Submit',
                icon: Icons.attach_file),
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 12),
              decoration: BoxDecoration(
                color: HelpColors.terracotta
                    .withValues(alpha: p.isDark ? 0.12 : 0.08),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(
                    color: p.faint, style: BorderStyle.solid, width: 1),
              ),
              child: Column(
                children: [
                  ...
                  Text(label,
                      style: const TextStyle(
                          ...
                          color: HelpColors.terracottaDeep)),
                ],
              ),
            ),
          ),
    ...
    return Column(
      ...
      children: [
        ...
        Row(children: [
          btn(Icons.image_outlined, 'Screenshot'),
          const SizedBox(width: 10),
          btn(Icons.videocam_outlined, 'Video')
        ]),
      ],
    );
  }
                ...
                children: [
                  Padding(
                    ...
                    child: Text(cat.label.toUpperCase(),
                        style: helpLabel(
                            size: 10, color: HelpColors.terracottaDeep)),
                  ),
                  for (final f in matches)
                    _FaqTile(
                      faq: f,
                      open: _open.contains(f.question),
                      onTap: () => setState(() => _open.contains(f.question)
                          ? _open.remove(f.question)
                          : _open.add(f.question)),
                    ),
          ...
          AnimatedCrossFade(
            ...
            secondChild: Padding(
              ...
              child: Text(faq.answer,
                  style:
                      TextStyle(fontSize: 12.5, height: 1.55, color: p.muted)),
            ),
            crossFadeState:
          ...
          HelpMenuList(children: [
            for (final it in items)
              HelpMenuItem(
                ...
                onTap: () =>
                    pushHelp(context, HelpArticleScreen(articleKey: it.$1)),
              ),
          ]),
        ],
      ),
          ...
          child: Column(
            ...
            children: [
              Text(label.toUpperCase(),
                  style: helpLabel(size: 10, color: HelpColors.terracottaDeep)),
              const SizedBox(height: 8),
              child,
            ],
          ),
      ...
      body: ListView(
        ...
        children: [
          block(
              'Problem',
              Text(a.problem,
                  style: TextStyle(fontSize: 13, height: 1.6, color: p.ink))),
          block(
              'Possible causes',
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
              ...
              Column(
                children: [
                  for (int i = 0; i < a.steps.length; i++)
                    _StepRow(number: i + 1, text: a.steps[i]),
                ],
              )),
          const SizedBox(height: 4),
          HelpSecondaryButton(
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
              child: Text(text,
                  style: TextStyle(fontSize: 13, height: 1.5, color: p.ink))),
        ],
      ),
    );
  }
    ...
    return HelpScaffold(
      ...
      body: ListView(
        ...
        children: [
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    height: 42,
                    decoration: BoxDecoration(
                        color: HelpColors.alert.withValues(alpha: 0.14),
                        borderRadius: BorderRadius.circular(12)),
                    child: const Icon(Icons.emergency_share_outlined,
                        size: 18, color: HelpColors.alert),
                  ),
                  ...
                  InteractiveSurface(
                    onTap: () => _launch(context, Uri.parse('tel:${e.number}'),
                        'Dial ${e.number}'),
                    child: Container(
                      width: 40,
                      height: 40,
                      decoration: const BoxDecoration(
                          color: HelpColors.sage, shape: BoxShape.circle),
                      child:
          ...
          Center(
              child: Text(
                  'If you are in immediate danger, move to a public place first.',
        ...
        children: [
          HelpMenuList(children: [
            ...
            HelpMenuItem(
              ...
              title: 'Phone',
              subtitle: helpSupportPhone,
              onTap: () => _launch(
                  context,
                  Uri.parse('tel:${helpSupportPhone.replaceAll(' ', '')}'),
                  'Call $helpSupportPhone'),
            ),
            HelpMenuItem(
                icon: Icons.schedule,
                title: 'Office hours',
                subtitle: helpOfficeHours),
            HelpMenuItem(
                icon: Icons.place_outlined,
                title: 'Office',
                subtitle: 'Banepa, Kavre, Nepal'),
          ]),
          const SizedBox(height: 14),
          HelpCard(
            onTap: () async {
              await Clipboard.setData(
                  const ClipboardData(text: helpSupportEmail));
              if (context.mounted) {
                showHelpToast(context, 'Email copied', icon: Icons.copy);
              }
            },
            child: Row(children: [
              const Icon(Icons.copy,
                  size: 16, color: HelpColors.terracottaDeep),
              const SizedBox(width: 10),
              Expanded(
                  child: Text('Copy support email',
                      style: TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.w600,
                          color: HelpPalette.of(context).ink))),
            ]),
          ),
        ],
      ),
...
class _HelpFeedbackScreenState extends State<HelpFeedbackScreen> {
  ...
  Future<void> _submit() async {
    if (_rating == 0) {
      showHelpToast(context, 'Tap a star to rate first',
          icon: Icons.info_outline);
      return;
    }
    setState(() => _sending = true);
    ...
    return HelpScaffold(
      ...
      body: ListView(
        ...
        children: [
          const SizedBox(height: 8),
          Center(
              child: Text('How would you rate Sampada?',
  ...
  Widget build(BuildContext context) {
    const items = [
      (
        ...
        'Conduct, no-show, misrepresentation'
      ),
      ('Report User', Icons.person_outline, 'Abuse, spam, impersonation'),
      (
        'Report Event',
        Icons.event_outlined,
        'Wrong details, cancelled, unsafe'
      ),
      (
        'Report Heritage Site',
        Icons.account_balance_outlined,
        'Incorrect info, closed, damaged'
      ),
      (
        'Report App Bug',
        Icons.bug_report_outlined,
        'Crashes, glitches, broken screens'
      ),
    ];
    return HelpScaffold(
          ...
          HelpMenuList(children: [
            for (final it in items)
              HelpMenuItem(
                ...
                title: it.$1,
                subtitle: it.$3,
                tint: HelpColors.alert,
                onTap: () =>
                    pushHelp(context, HelpReportFormScreen(kind: it.$1)),
              ),
          ]),
        ],
      ),
...
class _HelpReportFormScreenState extends State<HelpReportFormScreen> {
  ...
  Future<void> _submit() async {
    if (_details.text.trim().isEmpty) {
      showHelpToast(context, 'Add some detail so we can look into it',
          icon: Icons.info_outline);
      return;
    }
    setState(() => _sending = true);
              ...
              child: HelpCard(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(pt.body,
                        style: TextStyle(
                            fontSize: 12.5, height: 1.55, color: p.muted)),
                  ],
                ),
              ),
            ),
...
class _HelpLiveChatScreenState extends State<HelpLiveChatScreen> {
  ...
  final List<({bool me, String text})> _messages = [
    (
      me: false,
      text: 'Namaste! I\'m Sujata from Sampada support. How can I help today?'
    ),
  ];
    ...
    return HelpScaffold(
      ...
      body: Column(
        children: [
          ...
          Container(
            decoration: BoxDecoration(
                color: p.page, border: Border(top: BorderSide(color: p.line))),
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      focusedBorder: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(20),
                          borderSide:
                              const BorderSide(color: HelpColors.terracotta)),
                    ),
                  ),
                ),
  ...
  switch (s) {
    case 'resolved':
      return (
        ...
        fg: const Color(0xFF2D7A3A),
        label: 'Resolved'
      );
    case 'in_progress':
      return (
        bg: const Color(0xFFE8F0FB),
        fg: const Color(0xFF1B5FA8),
        label: 'In progress'
      );
    case 'closed':
      return (
        bg: const Color(0xFFEAE4DC),
        fg: const Color(0xFF6B5F57),
        label: 'Closed'
      );
    default:
      return (
...
class _HelpMyRequestsScreenState extends State<HelpMyRequestsScreen> {
  late final SupportService _svc =
      SupportService(apiClient: di.sl<ApiClient>());
              ...
              : tickets.isEmpty
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) =>
                            _ticketCard(context, p, tickets[i]),
                      ),
                    ),
    );
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              Text(
                  'When you contact support or report a problem, it appears here with our reply.',
                  textAlign: TextAlign.center,
                  style:
                      TextStyle(fontSize: 12.5, height: 1.5, color: p.muted)),
              const SizedBox(height: 20),
              HelpPrimaryButton(
                ...
                Expanded(
                  child: Text(
                    t.subject.isNotEmpty
                        ? t.subject
                        : (t.category.isNotEmpty ? t.category : 'Request'),
                    maxLines: 1,
            ...
            if (t.hasReply) ...[
              ...
              Row(children: [
                const Icon(Icons.mark_chat_read_outlined,
                    size: 14, color: HelpColors.sage),
                const SizedBox(width: 6),
                Text('Support replied',
                    style: TextStyle(
                        fontSize: 11.5,
                        fontWeight: FontWeight.w600,
                        color: HelpColors.sage)),
              ]),
            ],
          ],
        ),
                    ...
                    if (t.hasReply) ...[
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                              color:
                                  p.isDark ? p.line : const Color(0xFFCFE6D3)),
                        ),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                      ...
                      const SizedBox(height: 18),
                      Text(
                          'No reply yet — we\'ll notify you when support responds.',
...
void openAbout(BuildContext context) =>
    Navigator.pushNamed(context, AppStrings.aboutPath);
