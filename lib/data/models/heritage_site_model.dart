...
class HeritageSiteModel extends HeritageSite {
  ...
  factory HeritageSiteModel.fromJson(Map<String, dynamic> json) {
    ...
    final String categoryStr = categoryRaw is Map
        ? (categoryRaw['name_en'] ?? categoryRaw['slug'] ?? 'heritage')
            .toString()
    ...
    final String districtIdStr =
        districtRaw is Map ? (districtRaw['id'] ?? '').toString() : '';
    ...
    return HeritageSiteModel(
      ...
      nameNepali:
          json['name_np'] ?? json['name_ne'] ?? json['name_nepali'] ?? '',
      ...
      gallery: (json['gallery'] as List?)
              ?.map((i) => SiteImageModel.fromJson(i))
              .toList() ??
          [],
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      // Kept nullable until a detail endpoint supplies them.  SQLite stores
      // compact text/URLs only; images remain in the image cache.
      'history_en': null,
      'history_ne': null,
      'latitude': latitude,
      'longitude': longitude,
  ...
  factory HeritageSiteModel.fromMap(Map<String, dynamic> map) {
    return HeritageSiteModel(
      ...
      descriptionNepali:
          map['description_ne'] ?? map['description_nepali'] ?? '',
